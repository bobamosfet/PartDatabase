...
def insert_sample_data():
    ...
    sample_data = [
        ('PN001', 'A', 'Widget Assembly Base Unit', 'Assy-101, Assy-202', 'Active', now),
        ('PN002', 'B', 'Control Module v2', 'Product X, Product Y Rev C', 'Active', now),
        ('PN003', 'A', 'Sensor Housing Stainless', 'Assy-303, Test Fixture 4', 'Active', now),
        ('PN004', 'C', 'Gearbox 90deg 1:5 ratio', 'Robot Arm v3, Conveyor B', 'Obsolete', now),
        ('PN005', 'A', 'Power Supply 24V 5A', 'Control Cabinet A, Backup Unit', 'Active', now),
        ('PN006', 'A', 'Mounting Bracket Left', 'Assy-101, Assy-202', 'New', now),
        ('PN007', 'B', 'Mounting Bracket Right', 'Assy-101, Assy-202', 'Active', now),
        ('PN101', '1', 'Main Chassis Weldment', 'Product X Final, Product Y', 'Active', now),
    ]
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.executemany('''
        INSERT OR REPLACE INTO parts 
        (part_number, revision, description, where_used, status, last_updated)
        VALUES (?, ?, ?, ?, ?, ?)
    ''', sample_data)
    conn.commit()
    conn.close()
...
class PartsApp:
    def __init__(self, root):
        ...
        self.search_entries = {}
        fields = ['part_number', 'revision', 'description', 'where_used', 'status']

        for i, field in enumerate(fields):
            ttk.Label(filter_frame, text=field.replace('_', ' ').title() + ":").grid(row=i//2, column=(i%2)*3, padx=5, pady=3, sticky="e")
            entry = ttk.Entry(filter_frame, width=28)
            entry.grid(row=i//2, column=(i%2)*3 + 1, padx=5, pady=3)
            self.search_entries[field] = entry

        btn_frame = ttk.Frame(filter_frame)
        btn_frame.grid(row=len(fields)//2 + 1, column=0, columnspan=4, pady=10)
        ...
        # ── Treeview ────────────────────────────────────────────────────
        self.tree = ttk.Treeview(root, columns=('Part Number', 'Rev', 'Description', 'Where Used', 'Status', 'Last Updated'), show='headings')
        cols = {
            'Part Number': ('part_number', 110),
            'Rev': ('revision', 60),
            'Description': ('description', 220),
            'Where Used': ('where_used', 180),
            'Status': ('status', 90),
            'Last Updated': ('last_updated', 140)
        }
        for col, (db_col, width) in cols.items():
    ...
    def load_data(self, where_clause="", params=()):
        ...
        query = f"SELECT part_number, revision, description, where_used, status, last_updated FROM parts {where_clause} {order_by}"
        cursor.execute(query, params)
        rows = cursor.fetchall()
        conn.close()

        # Update sort arrows
        for col_display, (col_db, _) in {
            'Part Number': ('part_number',0), 'Rev': ('revision',0),
            'Description': ('description',0), 'Where Used': ('where_used',0),
            'Status': ('status',0), 'Last Updated': ('last_updated',0)
        }.items():
            arrow = ""
            if col_db == self.sort_column:
                arrow = " ↓" if self.sort_reverse else " ↑"
            self.tree.heading(col_display, text=col_display + arrow)
    ...
    def open_edit_window(self, is_new=True, values=None):
        ...
        entries = {}
        fields = ['part_number', 'revision', 'description', 'where_used', 'status']
        labels = ['Part Number*', 'Revision*', 'Description', 'Where Used', 'Status']

        for i, (field, label) in enumerate(zip(fields, labels)):
            ttk.Label(frame, text=label).grid(row=i, column=0, sticky="e", pady=6, padx=8)
            if field == 'status':
                combo = ttk.Combobox(frame, values=['New', 'Active', 'Obsolete'], width=30)
        ...
        if not is_new and values:
            for field, val in zip(['part_number', 'revision', 'description', 'where_used', 'status'], values[:5]):
                if field == 'status':
        ...
        def save():
            ...
            try:
                if is_new:
                    ...
                    cursor.execute('''
                        UPDATE parts SET
                            description = ?, where_used = ?, status = ?, last_updated = ?
    ...
    def show_details(self, event):
        ...
        labels = ['Part Number', 'Revision', 'Description', 'Where Used', 'Status', 'Last Updated']
        for i, (lbl, val) in enumerate(zip(labels, values)):
            ttk.Label(frame, text=lbl + ":", font=("Arial", 10, "bold")).grid(row=i, column=0, sticky="ne", padx=10, pady=6)
            ttk.Label(frame, text=val or "—", wraplength=500, justify="left").grid(row=i, column=1, sticky="nw", pady=6)
    ...
    def export_to_csv(self):
        ...
        with open(file_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            # Header
            writer.writerow(['Part Number', 'Revision', 'Description', 'Where Used', 'Status', 'Last Updated'])
            # Data
            for item in self.tree.get_children():
